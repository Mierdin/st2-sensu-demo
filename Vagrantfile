...
hostname   = ENV['HOSTNAME'] ? ENV['HOSTNAME'] : 'st2-sensu-demo'
box        = ENV['BOX'] ? ENV['BOX'] : 'ubuntu/trusty64'
st2user    = ENV['ST2USER'] ? ENV['ST2USER']: 'st2admin'
st2passwd  = ENV['ST2PASSWORD'] ? ENV['ST2PASSWORD'] : 'Password1!'
...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  config.vm.define "st2" do |st2|
    ...
    st2.vm.provision "shell" do |s|
      s.path = "scripts/install_sensu.sh"
      s.privileged = true
    end
    ...
    st2.vm.provision "shell" do |s|
      s.path = "scripts/st2_postinstall.sh"
      s.privileged = false
    end
